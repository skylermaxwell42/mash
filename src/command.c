...
struct Process* createProcess(char* process_string) {
  ...
  process->args[0] = malloc(sizeof(process_string));
  process->args[1] = malloc(sizeof(process_string));
  process->args[2] = malloc(sizeof(process_string));
  //if (process == NULL) {
  //  fprintf(stderr, "Could not allocate space for Process struct\n");
  //}
  //initProcess(process);
...
//  process->args = malloc(sizeof(char)*MAX_CMD_LENGTH*NUM_PROC_ARGS);
//
